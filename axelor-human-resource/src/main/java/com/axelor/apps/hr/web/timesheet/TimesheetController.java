...
public class TimesheetController {
		...
		try {
			...
			if (timesheet.getEmployee() == null) {
				throw new AxelorException(timesheet, TraceBackRepository.CATEGORY_CONFIGURATION_ERROR,
						I18n.get(HumanResourceExceptionMessage.LEAVE_USER_EMPLOYEE), AuthUtils.getUser().getName());
			}
			...
			timesheet = Beans.get(TimesheetService.class).generateLines(timesheet, fromGenerationDate, toGenerationDate,
	...
	public void editTimesheet(ActionRequest request, ActionResponse response) {
		List<Timesheet> timesheetList = Beans.get(TimesheetRepository.class).all()
				.filter("self.employee.user.id = ?1 AND self.company = ?2 AND self.statusSelect = 1",
						AuthUtils.getUser().getId(),
						Optional.ofNullable(AuthUtils.getUser()).map(User::getActiveCompany).orElse(null))
				.fetch();
		if (timesheetList.isEmpty()) {
			response.setView(ActionView.define(I18n.get("Timesheet")).model(Timesheet.class.getName())
					.add("form", "timesheet-form").map());
		} else if (timesheetList.size() == 1) {
			response.setView(ActionView.define(I18n.get("Timesheet")).model(Timesheet.class.getName())
					.add("form", "timesheet-form").param("forceEdit", "true")
					.context("_showRecord", String.valueOf(timesheetList.get(0).getId())).map());
		} else {
			response.setView(ActionView.define(I18n.get("Timesheet")).model(Wizard.class.getName())
					.add("form", "popup-timesheet-form").param("forceEdit", "true").param("popup", "true")
					.param("show-toolbar", "false").param("show-confirm", "false").param("forceEdit", "true")
					.param("popup-save", "false").map());
		}
	}
	...
	public void allTimesheet(ActionRequest request, ActionResponse response) {
		...
		ActionViewBuilder actionView = ActionView.define(I18n.get("Timesheets")).model(Timesheet.class.getName())
				.add("grid", "all-timesheet-grid").add("form", "timesheet-form")
				.param("search-filters", "timesheet-filters");

		if (employee == null || !employee.getHrManager()) {
			if (employee == null || employee.getManagerUser() == null) {
				actionView.domain("self.employee.user.id = :_userId OR self.employee.managerUser = :_user")
	...
	public void allTimesheetLine(ActionRequest request, ActionResponse response) {
		...
		ActionViewBuilder actionView = ActionView.define(I18n.get("See timesheet lines"))
				.model(TimesheetLine.class.getName()).add("grid", "timesheet-line-grid")
	...
	public void validateTimesheet(ActionRequest request, ActionResponse response) {
		...
		ActionViewBuilder actionView = ActionView.define(I18n.get("Timesheets to Validate"))
				.model(Timesheet.class.getName()).add("grid", "timesheet-validate-grid").add("form", "timesheet-form")
				.param("search-filters", "timesheet-filters")
				.context("todayDate", Beans.get(AppBaseService.class).getTodayDate(user.getActiveCompany()));
	...
	public void editTimesheetSelected(ActionRequest request, ActionResponse response) {
		...
		response.setView(ActionView.define(I18n.get("Timesheet")).model(Timesheet.class.getName())
				.add("form", "timesheet-form").param("forceEdit", "true").domain("self.id = " + timesheetMap.get("id"))
	...
	public void historicTimesheet(ActionRequest request, ActionResponse response) {
		...
		ActionViewBuilder actionView = ActionView.define(I18n.get("Historic colleague Timesheets"))
				.model(Timesheet.class.getName()).add("grid", "timesheet-grid").add("form", "timesheet-form")
				.param("search-filters", "timesheet-filters");

		actionView.domain("(self.statusSelect = 3 OR self.statusSelect = 4)");

		if (employee == null || !employee.getHrManager()) {
			actionView.domain(actionView.get().getDomain() + " AND self.employee.managerUser = :_user").context("_user",
					user);
		}
	...
	public void historicTimesheetLine(ActionRequest request, ActionResponse response) {
		...
		if (employee == null || !employee.getHrManager()) {
			actionView.domain(actionView.get().getDomain() + " AND self.timesheet.employee.managerUser = :_user")
					.context("_user", user);
		}
	...
	public void showSubordinateTimesheets(ActionRequest request, ActionResponse response) {
		...
		String domain = "self.employee.managerUser.employee.managerUser = :_user AND self.company = :_activeCompany AND self.statusSelect = 2";

		long nbTimesheets = Query.of(Timesheet.class).filter(domain).bind("_user", user)
				.bind("_activeCompany", activeCompany).count();
	...
	public void cancel(ActionRequest request, ActionResponse response) {
		try {
			...
			if (message != null && message.getStatusSelect() == MessageRepository.STATUS_SENT) {
				response.setInfo(String.format(I18n.get("Email sent to %s"),
	...
	/**
	 * Action called when confirming a timesheet. Changing status + Sending mail to
	 * Manager
	 *
	 * @param request
	 * @param response
	...
	public void continueBtn(ActionRequest request, ActionResponse response) {
		response.setView(
				ActionView.define(I18n.get("Timesheet")).model(Timesheet.class.getName()).add("form", "timesheet-form")
						.add("grid", "timesheet-grid").param("search-filters", "timesheet-filters")
						.domain("self.employee.user = :_user").context("_user", AuthUtils.getUser()).map());
	}
	...
	/**
	 * Action called when validating a timesheet. Changing status + Sending mail to
	 * Applicant
	 *
	 * @param request
	 * @param response
	...
	public void valid(ActionRequest request, ActionResponse response) {
		...
		try {
			...
			Beans.get(PeriodService.class).checkPeriod(timesheet.getCompany(), timesheet.getToDate(),
	...
	// action called when refusing a timesheet. Changing status + Sending mail to
	// Applicant
	...
	public String timesheetValidateMenuTag() {
		...
		return Beans.get(HRMenuTagService.class).countRecordsTag(Timesheet.class, TimesheetRepository.STATUS_CONFIRMED);
	}
	...
	public void printTimesheet(ActionRequest request, ActionResponse response) throws AxelorException {
		...
		String fileLink = ReportFactory.createReport(IReport.TIMESHEET, name).addParam("TimesheetId", timesheet.getId())
				.addParam("Timezone", timesheet.getCompany() != null ? timesheet.getCompany().getTimezone() : null)
				.addParam("Locale", ReportSettings.getPrintingLocale(null)).toAttach(timesheet).generate()
				.getFileLink();
	...
	public void openTimesheetEditor(ActionRequest request, ActionResponse response) {
		...
		response.setView(ActionView.define(I18n.get("Timesheet lines")).add("html", url).param("popup", "reload")
				.param("popup-save", "false").map());
	}
	...
	public void timesheetPeriodTotalController(ActionRequest request, ActionResponse response) {
		try {
			...
			response.setAttr("$periodTotalConvert", "value",
	...
	/**
	 * Called from timesheet form, on user change. Call
	 * {@link TimesheetService#updateTimeLoggingPreference(Timesheet)} to update the
	 * timesheet, and update the dummy field $periodTotalConvert
	 *
	 * @param request
	 * @param response
	 */
	public void updateTimeLoggingPreference(ActionRequest request, ActionResponse response) {
		try {
			...
			response.setAttr("$periodTotalConvert", "hidden", false);
			response.setAttr("$periodTotalConvert", "value", Beans.get(TimesheetLineService.class)
					.computeHoursDuration(timesheet, timesheet.getPeriodTotal(), false));
			response.setAttr("$periodTotalConvert", "title",
	...
	public void printTimeSheetReport(ActionRequest request, ActionResponse response) {
		String tsIdsStr = "";
		...
		tsIdsStr = Joiner.on(",").join(selectedFiels);

		List<TimesheetLine> timesheetLineList = Beans.get(TimesheetLineRepository.class).all()
				.filter("self.timesheet.id in (" + tsIdsStr + ")").fetch();

		List<Long> idList = timesheetLineList.stream().map(TimesheetLine::getId).collect(Collectors.toList());

		String idsStr = Joiner.on(",").join(idList);
		try {
			String fileLink =
			          ReportFactory.createReport("ZdentalTimesheet.rptdesign", "TimesheetReport" + "-${date}")
			              .addParam("ids", idsStr)
//			              .addFormat("xls")
			              .generate()
			              .getFileLink();
		      response.setView(ActionView.define("Timesheet").add("html", fileLink).map());
