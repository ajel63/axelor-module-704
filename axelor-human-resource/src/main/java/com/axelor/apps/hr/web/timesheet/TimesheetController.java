...
public class TimesheetController {
  ...
  public void printTimeSheetReportPdf(ActionRequest request, ActionResponse response) {
    String tsIdsStr = "";
    ...
    List<TimesheetLine> timesheetLineList =
        Beans.get(TimesheetLineRepository.class)
            .all()
            .filter("self.timesheet.id in (" + tsIdsStr + ")")
            .fetch();

    List<Long> idList =
        timesheetLineList.stream().map(TimesheetLine::getId).collect(Collectors.toList());
  ...
  public void printTimeSheetReportXls(ActionRequest request, ActionResponse response) {
    String tsIdsStr = "";
    if (request.getContext().get("_ids") == null) {
      response.setAlert("Please select the grid.");
      return;
